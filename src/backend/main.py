...
def generate_site(session_id: str = Query(...), prompt: str = Query(...)):
    session_data = sessions.get(session_id, {"html": ""})
    previous_html = session_data["html"]
    
    full_prompt = system_prompt + "\n\nUser prompt: " + prompt + "\n\nPrevious HTML:\n" + previous_html + "\n\nHTML output:"
    response = llm(full_prompt, max_tokens=4096, temperature=0.3)
    html = response['choices'][0]['text'].strip()
    
    sessions[session_id] = {"html": html}
    return {"response": html}


@app.get("/retrieve")
def retrieve_site(session_id: str):
    session_data = sessions.get(session_id, {"html": ""})
...
@app.post("/edit")
def edit_site(session_id: str, new_prompt: str = Query(...)):
    session_data = sessions.get(session_id, {"html": ""})
    current_html = session_data["html"]
    
    full_prompt = system_prompt + "\n\nUser prompt: " + new_prompt + "\n\nCurrent HTML:\n" + current_html + "\n\nUpdated HTML output:"
    response = llm(full_prompt, max_tokens=4096, temperature=0.3)
    updated_html = response['choices'][0]['text'].strip()
    
    sessions[session_id] = {"html": updated_html}
